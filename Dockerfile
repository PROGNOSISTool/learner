...
RUN apk add --no-cache bash apache-ant curl
...
ENTRYPOINT ["wait-for-it", "adapter:3333", "-s", "--", "java", "-cp", "Learner/dist/QUICLearner.jar:Learner/lib/*", "learner.Main", "config.yaml"]
